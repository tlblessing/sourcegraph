 ...
 */
export const UserAreaSidebar: React.FunctionComponent<Props> = ({ url, className = '', ...props }) => (
    <div className={`user-area-sidebar d-flex flex-column ${className}`}>
        {props.user.avatarURL && (
            <UserAvatar className="user-area-sidebar__avatar align-self-center mb-2" user={props.user} />
        )}
        {props.user.displayName ? (
            <header>
                <h1 className="test-user-area-sidebar__display-name h2 font-weight-bold">{props.user.displayName} </h1>
        ...
        {props.user.viewerCanAdminister && (
            <Link to={props.user.settingsURL!} className="btn btn-link align-self-start pl-0 d-flex align-items-center">
